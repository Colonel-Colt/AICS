...
def trainCNN(train_path, classes, image_size = 32,  model_file = 'model.pkl', download_flag = False):
	...
	X, Y, ids, cls, testX, testY, ids, cls, num= dataset.load_train(train_path,image_size,classes,10)
...
def train(username):
	...
	try:
		os.remove(os.path.join(os.getcwd(), username, "trainOK.cnn"))
	except:
		print ("trainOK.cnn not existed")

	#try:
	image_size, classes = urlParser.parse(urlFile, train_path)
	#except:
	#	print "Parse url fail {}".format(urlFile)
	#	return False
#	print classes
	try:
		...
		print "write classes file error"
		return False
	
	model_file = os.path.join(os.getcwd(), username, 'cnn.model')
	try:
		trainCNN(train_path, classes, image_size, model_file, True)
	except:
		print "Train CNN error"
		return False
	try:
		open(os.path.join(os.getcwd(), username, 'trainOK.cnn'), "w")
	except:
		print "Cannot write trainOK.cnn"
		return False
	return True
#train('temp')
